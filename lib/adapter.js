...
const Adapter = {
  ...
  setupConnectionSockets (connection, model, config) {
    return Promise.all([
      ...
      this.getPushSocket(connection.identity, model.identity, { worker: 'persistence' })
        .then(pushSocket => {
  ...
  getPushSocket (connection, collection, options = { }) {
    let config = this.connections.get(connection)
    let context = config.context
    let address = this.getQueueName(collection, options.worker)
  ...
  getQueueName (model, worker) {
    if (_.isUndefined(worker)) {
      throw new Error('worker cannot be undefined in getQueueName')
    }
