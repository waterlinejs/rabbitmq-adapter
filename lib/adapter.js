...
function setupConnection(config, collections, context) {
  return Promise.map(collections, function (model) {
    return new Promise((resolve, reject) => {
      var publish = context.socket('PUBLISH', { routing: 'topic' });

      pub.connect(model.identity, function () {
        resolve(publish);
      })
    })
  })
...
var Adapter = {
  ...
  registerConnection: function(config, collections, cb) {
    ...
    var context = rabbit.createContext(config.url);

    context.on('ready', function () {

      connections[config.identity] = setupConnection(config, collections, context);
    });
  },
